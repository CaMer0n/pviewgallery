...
if (count($nonapproved_Images)) {
	...
	foreach ($nonapproved_Images as $dataset) {
		...
		$ImageSize = getimagesize($PView -> getOrigPath($dataset['imageId']));
		$FileSize = round(filesize($PView -> getOrigPath($dataset['imageId'],"REL"))/1024,2);
		...
		$out_HTML.= "<tr><td style='vertical-align:top;' class='forumheader3'><img src='".$PView -> getThumbPath($dataset['imageId'])."'><br />".$ImageSize[0]."x".$ImageSize[1]."&nbsp;".LAN_IMAGE_15."<br />".$FileSize." kB<br /><br />".LAN_ADMIN_53.": <a href='pviewgallery.php?album=".$dataset['albumId']."' target='_blank'>".$tp -> toHTML($PView -> getAlbumName($dataset['albumId']))."</td>";
